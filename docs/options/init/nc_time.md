...
In the next example two lines from the -nc\_table file instruct
...
>Description: init  X      netcdf, [[-]yyyymmddhhnnss]:[dt{s[ec]|m[in]|h[our]|d[ay]}], [-] is for time alignment only

_Docs derived from <https://www.cpc.ncep.noaa.gov/products/wesley/wgrib2/nc_time.html>_